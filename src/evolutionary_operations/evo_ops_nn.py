...
class EvolutionaryOperationsNN(EvolutionaryOperations):
    @staticmethod
    def crossover(parent_1_keep_precentage, parent_1_layers, parent_2_layers, child_layers):
        for i in range(len(child_layers)):
            assert child_layers[i].trainable, f"Network layer {child_layers[i]} is NOT trainable."
    ...
    @staticmethod
    def mutate(mutation_layers: List, mean: float = 0., variance: float = 0.0001):
