...
INITIAL_LEARNING_RATE = 1e-3  # Will not be used.
MODIFIED_LEARNING_RATE = 1e-4  # Will be used.
...
TRAINING_META_DATA_DIR = os.path.join(os.path.dirname(
    os.path.dirname(__file__)), "training_metadata_pong_v4")
...
agents_info = [{'name': 'g_8', 'generation': 6},
               {'name': 'g_11', 'generation': 5}]

assert len(
    agents_info) == POPSIZE, f"agents_info must provide info for {POPSIZE} agents."

for i in range(POPSIZE):
    network = network_factory.get_network()
    optimizer = tf.keras.optimizers.Adam(learning_rate=INITIAL_LEARNING_RATE)
    train_step_counter = tf.Variable(0)
    tf_agent = agent_factory.get_agent(name=agents_info[i]['name'], network=network,
                                       optimizer=optimizer,
    ...
    meta_agent.generation = agents_info[i]['generation']

    new_optimizer = tf.keras.optimizers.Adam(
        learning_rate=MODIFIED_LEARNING_RATE)

    meta_agent.tf_agent._optimizer = new_optimizer
